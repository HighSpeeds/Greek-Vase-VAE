...
class train_meter:
    ...
    def update(self, loss, recon_loss, kl_loss):
        self.loss+=loss
...
class val_meter:
    ...
    def update(self, loss, recon_loss, kl_loss,latent_space,ground_truth_images,recon_images):
        self.loss+=loss
        self.recon_loss+=recon_loss
        self.kl_loss+=kl_loss
    ...
    def plot_latent_space(self,save_path):
        ...
        #wandb.log({save_path[save_path.rfind("/")+1:save_path.rfind(".")]: fig})
        plt.close(fig)
    ...
    def plot_images(self,n_plot=4,save_path=""):
        ...
        ground_truth_images=np.concatenate([self.ground_truth_images[i][0] for i in indices])
        recon_images=np.concatenate([self.recon_images[i][0] for i in indices])
        #plot out in a grid
        fig,ax=plt.subplots(n_plot,2,figsize=(10,5*n_plot))
        cols=["Ground Truth","Recon"]
